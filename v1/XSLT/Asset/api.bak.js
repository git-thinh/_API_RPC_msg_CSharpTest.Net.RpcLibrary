        ...
        popup_login: function (msg) {
            ...
            if (_pop_allow) {
                ...
                var cf_login_options = {
                    ...
                    open: function (pid) {
                        api.log(pid);
                        //$('#' + pid + ' button[name=Save]').val('Đăng nhập');
                        //$('#' + pid + ' button[name=Clear]').val('Làm lại');
                        $('#' + pid + ' .w2ui-buttons').html('<button name="Save" class="w2ui-btn w2ui-btn-blue" style="">Đăng nhập</button><button name="Clear" class="w2ui-btn " style="">Làm lại</button>');
                    },
                    form: {
                        name: frm_name,
                        overflow: 'hidden',
                        fields: [
                            { field: 'username', type: 'text', required: true, html: { caption: 'Tài khoản', attr: ' style="width: 200px"' } },
                            { field: 'password', type: 'password', required: true, html: { caption: 'Mật khẩu', attr: ' style="width: 200px"' } }
                        ],
                        record: {
                            username: 'admin',
                            password: 'admin'
...
var textArea_htmlDecode = document.createElement('textarea');
es.onmessage = function (e) {
    var data___ = e.data.toString().trim();
    if (data___ != '' && data___ != 'data:' && data___ != 'ZGF0YTo=') {
        //console.log(data___);

        if (data___.indexOf('{') == 0) {
            var o = JSON.parse(data___);
            console.log(o);
            var mid = o['msg_id'];
            if (mid != null) {
                var lock = sessionStorage[mid + '.lock'];
                if (lock == 'true')
                    api.api_unlock();
                ...
                var _control = o['control'] == null ? '' : o['control'];
                var _action = o['action'] == null ? '' : o['action'];

                var dt = o['data'] == null ? [] : o['data'];
                switch (_control) {
                    case 'user':
                        switch (_action) {
                            case 'login':
                                if (dt == '' || dt.length == 0) {
                                    api.api_unlock();
                                    api.alert('Đăng nhập không thành công. Vui lòng đăng nhập lại');
                                } else {
                                    sessionStorage['user.username'] = dt[0]['username'];
                                    sessionStorage['user.password'] = dt[0]['password'];
                                    api.popup_login_close();
                                }
                                break;
                        }
                        break;
                ...
                var data_type = o['data_type'];
                if (data_type.indexOf('js_fun.') == 0) {
                    var fnName = data_type.substring(7, data_type.length);
                    if (eval('typeof ' + fnName) === "function")
                        window[fnName](dt);
                } else {
                    switch (data_type) {
                        case 'api_config':
                            var key_tag = data_type + '_' + _control + '_' + _action;
                            var it = document.getElementById(key_tag);
                            if (it == null) {
                                var js = o['data'];
                                js = window.atob(js);

                                textArea_htmlDecode.innerHTML = js;
                                js = textArea_htmlDecode.value;

                                $('body').append('<input id="' + key_tag + '" value="' + _control + '_' + _action + '" type="hidden" />' +
                                    '<script> ' + js + ' <' + '/' + 'script>');
                            }

                            if (_control == 'layout' && _action == 'main') {
                                o['control'] = 'tab';
                                o['action'] = 'email';
                                api.api_redirect(o, msLAYER.API_CONFIG);
                            } else {
                                var fnName = _control + m_split_controlAction + _action + '_reload';
                                if (eval('typeof ' + fnName) === "function")
                                    window[fnName](o);
                            }
                            break;
                ...
                sessionStorage.removeItem(mid + '.data');
                sessionStorage.removeItem(mid + '.msg');
                sessionStorage.removeItem(mid + '.lock');

                if (o['OK'] == true) {
                    var re_control = o['control_redirect'], re_action = o['action_redirect'];
                    if (re_control != null && re_action != null && re_control != '' && re_action != '') {
                        o['control'] = re_control;
                        o['action'] = re_action;
                        api.api_redirect(o);
                    }
