...
namespace CSharpTest.Net.RpcLibrary
{
    ...
    public class LurchTable<TKey, TValue> : IDictionary<TKey, TValue>, IDictionaryEx<TKey, TValue>, IConcurrentDictionary<TKey, TValue>
    {
        ...
        public void Initialize()
        {
            lock (this)
            {
                ...
                for (int slot = 0; slot < FreeSlots; slot++)
                {
                    ...
                    if (index != slot + 1)
                        throw new Exception("The LurchTable internal datastructure appears to be corrupted.");// new LurchTableCorruptionException();
